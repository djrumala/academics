...
<body background="images/cityup.jpg" style="background-size:cover; opacity:0.9">
	...
	<?php
		...
		if(isset($_GET['e'])){ //mengedit data
			...
			$queri="UPDATE tb_dosen set dosen='$nama' WHERE nip ='$nip'";
			$proses=mysqli_query($con,$queri);
			if($proses) header('location:pelatih.php');
			//echo "<script>Materialize.toast('Data diperbaharui', 1000)</script>;";
		}
		if(isset($_GET['a'])){ //menambah data		
			$nip=$_POST['nip'];
			$nama=$_POST['nama'];
			$query = "INSERT INTO tb_dosen VALUES('$nip','$nama',0)";
			$process = mysqli_query($con,$query);
			if($process) header('location:pelatih.php');			
		...
		if(isset($_GET['m'])){ //menghapus satu data		
			...
			if($row==0){
				...
				$query = "UPDATE tb_dosen SET flag=1 WHERE nip='$id'";
				$process = mysqli_query($con,$query);
				if($process) header('location:pelatih.php');
			}
			//echo "<script>Materialize.toast('".$row."', 1000)</script>;";
	...
	<!--UI Web-->	
			...
			<div class="col l12 m12 s12" style="position:fixed; opacity:0.8; bottom: 0px; left:80px; right:80px; padding:40px; text-align:right;">
				<div class="col l12 m12 s12">
					<a  href="berpel.php" class="col l1 btn ts modal-trigger" style="margin:2px; margin-right:310px;">Berkas Pelatih</a>
			...
			<div class="col l12 m12 s12" style="position:fixed; top:134px; bottom:0 left:80px; right: 1005px; font-size: 26px; font-family: 'Russo One'; color:#000; text-align:center;">
			DAFTAR PELATIH AKTIF</div>
		
	
	<!--Mengetahui banyak data di database-->	
	<?php
		include 'connect.php';
		$queri="SELECT * from tb_dosen WHERE flag=0";
		$proses=mysqli_query($con,$queri);
	...
	function edit(){ //edit
		...
		$(document).ready(function(){
				$("#edit").load("pelatihedit.php?sort="+srt+"&mark="+vup+"&pg="+page); //membuka form edit dan menyalin datanya sesuai penanda ke form edit
		});
		$('#add').hide();
		$('#minus').hide();
		$('#deleteall').hide();
	...
	function add(){ //tambah
		$('#add').show();
		$(document).ready(function(){
				$("#add").load("pelatihadd.php?"); //membuka tambah.php untuk menampilkan form kosong
		});
		$('#edit').hide();
		$('#minus').hide();
		$('#deleteall').hide();
	...
	function minus(){ //hapus
		$('#minus').show();
		$(document).ready(function(){
				$("#minus").load("pelatihdel.php?sort="+srt+"&mark="+vup+"&pg="+page); //membuka minus.php untuk menampilkan form kosong
		});
		$('#add').hide();
		$('#edit').hide();
		$('#deleteall').hide();
	...
	function upclick(){
		...
		$(document).ready(function(){
				$("#data").load("pelatihdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //header nama diklik akan membuka data yang mengurutkan berdasar nama
		});
	}
	...
	$(document).ready(function()
	{
			$("#data").load("pelatihdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //membuka pelatihdata.php untuk isi table
	});
		...
		function sortid()
		{
			$(document).ready(function()
			{srt="id";
				$("#data").load("pelatihdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //header email diklik akan membuka data yang mengurutkan berdasar email
			});
		}
