...
<body background="images/cityup.jpg" style="background-size:cover; opacity:0.9">
	...
	<?php
		...
		if(isset($_GET['r'])){ //mengembalikan satu data
			...
			$queri="UPDATE tb_mhs set flag=0 WHERE nrp='$nrp'";
			$proses=mysqli_query($con,$queri);
			if($proses) header('location:berkas.php');
		...
		if(isset($_GET['l'])){ //mengembalikan satu data
			...
			$queri="UPDATE tb_mhs set flag=0 WHERE flag=1";
			$proses=mysqli_query($con,$queri);
			if($proses) header('location:berkas.php');
		}	
		if(isset($_GET['m'])){ //menghapus satu data		
			...
			if($row==0){
				...
				$query = "DELETE FROM tb_mhs WHERE nrp='$id'";
				$process = mysqli_query($con,$query);
				if($process) header('location:berkas.php');
			}
		...
		if(isset($_GET['d'])){ //menghapus semua data	
			...
			if($row==0){
				...
				$queri="DELETE FROM tb_mhs WHERE flag=1";
				$proses=mysqli_query($con,$queri);
				if($proses) header('location:berkas.php');
			}
		}		
	...
	<!--UI Web-->	
			...
			<div class="col l12 m12 s12" style="position:fixed; bottom: 0px; left:80px; right:80px; padding:40px; text-align:right;">
				<div class="col l12 m12 s12">
					<a 	href="berkas.php?l=1" class="col l1 btn ts modal-trigger"  style="margin:2px;">Kembalikan Semua</a>
			...
			<div class="col l12 m12 s12" style="position:fixed; top:134px; bottom:0 left:80px; right: 1120px; font-size: 26px; font-family: 'Russo One'; color:#000; text-align:center;">
			BERKAS MAHASISWA</div>
		
	
	<!--Mengetahui banyak data di database-->	
	<?php
		include 'connect.php';
		$queri="SELECT * from tb_mhs WHERE flag=1;";
		$proses=mysqli_query($con,$queri);
	...
	function add(){ //mengembalikan satu data
		...
		$(document).ready(function(){
				$("#add").load("berkasrestore.php?sort="+srt+"&mark="+vup+"&pg="+page);
		});
		$('#edit').hide();
		$('#minus').hide();
		$('#deleteall').hide();
	...
	function minus(){ //hapus satu data
		$('#minus').show();
		$(document).ready(function(){
				$("#minus").load("berkasdelete.php?sort="+srt+"&mark="+vup+"&pg="+page);
		});
		$('#add').hide();
		$('#edit').hide();
		$('#deleteall').hide();
	...
	function upclick(){
		...
		$(document).ready(function(){
				$("#data").load("berkasdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //header nama diklik akan membuka data yang mengurutkan berdasar nama
		});
	}
	...
	$(document).ready(function()
	{
			$("#data").load("berkasdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //membuka data.php untuk isi table
	});
		...
		function sortid()
		{
			$(document).ready(function()
			{srt="id";
				$("#data").load("berkasdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //header email diklik akan membuka data yang mengurutkan berdasar email
			});
		}
		function sortdate()
		{
			...
			{
				srt="dt";
				$("#data").load("berkasdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //header alamat diklik akan membuka data yang mengurutkan berdasar alamat
			});
		}
		function sortvoice()
		{
			...
			{
				srt="vc";
				$("#data").load("berkasdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //header nope diklik akan membuka data yang mengurutkan berdasar suara
			});
		}
		function sortprof()
		{
			...
			{
				srt="pf";
				$("#data").load("berkasdata.php?sort="+srt+"&mark="+vup+"&pg="+page); //header nope diklik akan membuka data yang mengurutkan berdasar pelatih
			});
		}
